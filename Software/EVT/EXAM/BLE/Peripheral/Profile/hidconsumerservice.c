...
static gattAttribute_t hidAttrTbl[] = {
    ...
    /* HID report characteristic, Keyboard input declaration */
    {
        {ATT_BT_UUID_SIZE, characterUUID},
        GATT_PERMIT_READ,
        0,
...
uint8_t Hid_SetParameter(uint8_t id, uint8_t type, uint16_t uuid, uint8_t len, void *pValue)
{
    ...
    switch(uuid)
    {
        case REPORT_UUID:
            ...
            {
                PRINT("HID_REPORT_TYPE_INPUT, id = %d, len = %d\r\n", id, len);
                for (size_t i = 0; i < len; i++)
                {
                    PRINT("%02X ", (uint8_t *)pValue);
                    pValue++;
                }
                PRINT("\r\n");
            }
            ...
            {
                #if 0
                PRINT("HID_REPORT_TYPE_OUTPUT, id = %d, len = %d\r\n", id, len);
                for (size_t i = 0; i < len; i++)
                {
                    PRINT("0x%02X ", *((uint8_t *)pValue));
                    pValue++;
                }
                PRINT("\r\n");
                #endif
                /* NumLock bit0, CapsLock bit1, ScrollLock bit2 */
                pValue++;
                uint8_t led_status = *((uint8_t *)pValue);
                PRINT("NumLock[%c] CapsLock[%c] ScrollLock[%c]\r\n", led_status&0x01? '*' : ' ', led_status&0x02? '*' : ' ', led_status&0x04? '*' : ' ');
            }
            else
            {
        ...
        default:
            PRINT("Unknown attribute uuid = %d\r\n", uuid);
            // ignore the request
            break;
    }
